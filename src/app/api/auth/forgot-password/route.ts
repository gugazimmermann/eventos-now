...
export async function POST(request: Request) {
  ...
  if (!parseResult.success) {
    authLogger.warn("Password reset code validation error", { issues: parseResult.error.issues });
  ...
  try {
    ...
    authLogger.info("Password reset code sent successfully", { email: data.email });
    return NextResponse.json({ 
      success: true, 
      email: data.email,
    });
